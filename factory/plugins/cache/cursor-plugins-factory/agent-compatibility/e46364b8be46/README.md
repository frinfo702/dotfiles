...
Factory Droid plugin for checking how well a repo holds up under agent workflows. It pairs the published `agent-compatibility` CLI with focused reviews for startup, validation, and docs reliability.
...
```bash
~/.factory/plugins/local/agent-compatibility
```
