...
Look for `*-mode/SKILL.md` matching the user's handle, under the project's `.factory/skills/` or `~/.factory/skills/`. If one exists, confirm intent with `AskQuestion` (unless they already said "update my skill" or similar):
...
- Path: `.factory/skills/<handle>-mode/SKILL.md` in the project (or `~/.factory/skills/<handle>-mode/` if the user prefers a personal skill).
