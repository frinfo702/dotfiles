# Load machine-local environment variables and secrets.
typeset zsh_config_dir="${${(%):-%x}:A:h}"
[[ -r "$zsh_config_dir/.zshrc.local" ]] && source "$zsh_config_dir/.zshrc.local"
...
# Prompt format
PROMPT='${GREEN}%n${RESET} ${BLUE}%1~${RESET} ${YELLOW}${vcs_info_msg_0_}${RESET}
> '
...
function ghq-fzf() {
  local src=$(ghq list --full-path | fzf --preview "ls -laTp {1}")
  if [ -n "$src" ]; then
    BUFFER="cd $src"
    zle accept-line
  fi
...
zle -N ghq-fzf
bindkey '^g' ghq-fzf  # Binds to Ctrl+g

alias sshserver='TERM=xterm ssh -J frinfo702@100.124.199.38 frinfo@10.40.18.155'

# bun completions
[ -s "/Users/ken702/.bun/_bun" ] && source "/Users/ken702/.bun/_bun"
