...
## Script Paths (Grok plugin)

All CLI examples below are relative to **this skill directory** (the folder that contains `SKILL.md`, `scripts/`, and `data/`).

When running from another working directory, use the absolute skill path:

```bash
python3 <path-to-this-skill>/scripts/search.py "<query>" --design-system
```

In Grok, the skill path is the directory of this installed plugin skill (see skill inventory / `GROK_PLUGIN_ROOT` when loaded as a plugin).
