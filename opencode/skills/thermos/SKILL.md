---
name: thermos
description: Launch both thermo-nuclear review subagents (deep review + code quality) in parallel, then synthesize their findings. Use for combined bug/security and code-quality branch audits before merging.
---

# Thermos

Run both thermo review passes in parallel as background subagents, then synthesize their results.
...
3. Launch both subagents in the same message using the Task tool:
...
If individual background summaries are already visible, do not restate them wholesale. Surface the unified verdict, the highest-signal findings, and any remaining uncertainty.
